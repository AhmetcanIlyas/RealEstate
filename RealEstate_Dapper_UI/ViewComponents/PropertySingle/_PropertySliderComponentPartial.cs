...
namespace RealEstate_Dapper_UI.ViewComponents.PropertySingle
{
    public class _PropertySliderComponentPartial:ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(int id)
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:44331/api/ProductImages?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<GetPropertyImageByID>>(jsonData);
                return View(values);
            }
            return View();
        }
